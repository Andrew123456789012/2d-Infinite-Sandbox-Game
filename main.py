...
cactus = f"{Fore.GREEN}⍦ {Fore.WHITE}"
...
biomes = [
  [grass, empty, x_block, spike, chest, shop, grave],#plains
  [empty, rock, x_block, cactus, pyramid],#desert
  [grass, x_block, spike, chest, shop, grave],#dense forest
  [grass, spike, x_block, grave, empty],#graveyard
  [grass, x_block, spike, chest, shop, grave],#forest
  [0, 1, 2, 3, 4],#all biomes
]

biome_weights = [
  [18, 2, 5, 1, 0.1, 0.01, 0.01],#plains
  [1, 1, 0.5, 0.5, 0.005],#desert
  [1, 1, 1, 0.5, 0.005, 0.005],#dense forest
  [1, 0.5, 0.5, 0.5, 1],#graveyard
  [3, 1, 1, 0.05, 0.005, 0.005],#forest
  [1, 0.1, 0.05, 0.05, 0.5],#all biomes
]
    ...
    for i in range(y - self.radius, y + self.radius + 1):
      ...
      for j in range(x - self.radius, x + self.radius + 1):
        if self.border == 0 and self.ground == grass:
          biome_location = (round(i/12), round(j/12))
...
def game_start():
  ...
  u = World([x_block, empty, stone_floor, spike, ore, chest, shop, exit, rock],
            [4, 0.5, 1.5, 0.5, 0.5, 0.01, 0.005, 0.01, 1], 5, stone_floor, exit, 0)
...
def player_move(p, g, u, w, i, move):
  p.candidate_move = p.coordinates.copy()#movement
  ...
  if move == "w":
    ...
    if w.get_board_value(p.candidate_move) == w.ground or w.get_board_value(p.candidate_move) == empty:
      ...
      if i._bag[int(move) - 1].name == "CACTUS":
        w.set_board_value(p.candidate_move, cactus)
    ...
    if w.get_board_value(p.candidate_move) == cactus and p.pickaxes > 0:
      w.set_board_value(p.candidate_move, empty)
      p.pickaxes -= 1
      i.add_item(Item("CACTUS", 1, 5))
  ...
  if move == "w" or move == "a" or move == "s" or move == "d":
    ...
    elif w.get_board_value(p.candidate_move) == cactus:
      p.health -=1
